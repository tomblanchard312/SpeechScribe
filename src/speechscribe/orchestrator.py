...
class SpeechScribeOrchestrator:
    ...
    def process_session(
        ...
        **source_kwargs
        ...
        logger.info(
            f"Starting session {session_id} with profile {profile_name}")
        ...
        try:
            ...
            if not adapter.connect():
                raise RuntimeError(
                    f"Failed to connect to {source_type} source")
            ...
            if delivery_manager.deliver_all(segments, metadata):
                ...
                logger.warning(
                    f"Session {session_id} completed with delivery issues")
    ...
    def _configure_delivery(self, delivery_manager: DeliveryManager,
                            delivery_config: Dict[str, Any]):
        ...
        if 'live_caption_url' in delivery_config:
            delivery_manager.add_live_captions(
                delivery_config['live_caption_url'])
