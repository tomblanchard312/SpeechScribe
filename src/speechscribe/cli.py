...
@click.option(
    "--translate", "-t", is_flag=True, help="Translate non-English speech to English"
)
@click.option("--language", "-l", help="Force specific language code (e.g., en, es)")
@click.option("--no-convert", is_flag=True, help="Skip audio conversion to WAV")
@click.option(
    "--output-dir",
    "-o",
...
@click.pass_context
def transcribe(
    ctx, audio_file, model, device, translate, language, no_convert, output_dir, formats
):
    ...
    try:
        ...
        try:
            ...
            output_files = manager.process_single_file(
                audio_file, output_dir=output_dir, no_convert=no_convert
            )
...
@click.option(
    "--device", "-d", type=click.Choice(["cpu", "cuda"]), help="Inference device"
)
@click.option(
    "--translate", "-t", is_flag=True, help="Translate non-English speech to English"
)
...
@click.option(
    "--pattern",
    "-p",
    default="*.mp3,*.m4a,*.wav,*.flac,*.mov,*.mp4",
    ...
    try:
        ...
        try:
            ...
            results = manager.batch_transcribe(
                audio_files, output_dir=output_dir, no_convert=no_convert
            )
...
@cli.command()
@click.option(
    "--config-path", type=click.Path(path_type=Path), help="Path to configuration file"
)
@click.pass_context
...
@click.option(
    "--emotion",
    type=click.Choice(["Happy", "Sad", "Angry", "Fearful", "Disgusted", "Surprised"]),
...
@click.option(
    "--emphasis",
    type=click.Choice(["weak", "normal", "strong"]),
    default="normal",
    ...
    try:
        ...
        if voice != "default":
            ...
            if voice in voice_names:
                ...
                output_path = synthesizer.use_trained_voice(
                    voice, text, output, fix_speed=not no_fix_speed
                )
                ...
                click.echo(
                    f"⚠️  Voice '{voice}' not found in trained voices, using default TTS"
                )
...
@click.pass_context
def clone_voice(
    ctx, source_audio, text, output, engine, no_fix_speed, quality, emotion
):
    ...
    try:
        ...
        output_path = synthesizer.clone_voice(
            source_audio, text, output, engine, **kwargs
        )
...
def convert_voice(ctx, source_audio, target_voice, output, engine, no_fix_speed):
    ...
    try:
        ...
        if not synthesizer.available_engines.get(engine, False):
            click.echo(
                f"❌ Voice conversion engine {engine} is not available.", err=True
            )
            sys.exit(1)

        # Convert voice
        click.echo(
            f"🔄 Converting voice from {source_audio.name} to {target_voice.name}..."
        )

        # Add speed fixing option
        kwargs = {"fix_speed": not no_fix_speed}
        output_path = synthesizer.convert_voice(
            source_audio, target_voice, output, engine, **kwargs
        )
...
@click.option(
    ...
    default="high",
    help="Voice quality level (ultra for maximum realism)",
)
@click.option(
    "--min-duration",
    default=5.0,
...
@click.option(
    "--max-duration",
    default=300.0,
    help="Maximum audio duration in seconds (default: 300.0 = 5 minutes)",
)
@click.option(
    "--recursive", "-r", is_flag=True, help="Process subdirectories recursively"
)
    ...
    try:
        ...
        for file_path in audio_files:
            try:
                ...
                if (
                    audio_info
                    and min_duration <= audio_info["duration"] <= max_duration
                ):
                    valid_files.append((file_path, audio_info["duration"]))
                    click.echo(
                        f"✅ Accepting {file_path.name} (duration: {audio_info['duration']:.1f}s)"
                    )
                else:
                    if audio_info:
                        if audio_info["duration"] < min_duration:
                            click.echo(
                                f"⚠️  Skipping {file_path.name} (too short: {audio_info['duration']:.1f}s < {min_duration:.1f}s)"
                            )
                        else:
                            click.echo(
                                f"⚠️  Skipping {file_path.name} (too long: {audio_info['duration']:.1f}s > {max_duration:.1f}s)"
                            )
                    else:
                        click.echo(
        ...
        if not valid_files:
            ...
            click.echo(
                f"TIP: Try adjusting duration limits: --min-duration {min_duration/2:.1f} --max-duration {max_duration*2:.1f}"
            )
            sys.exit(1)
        ...
        click.echo(
            f"🎤 You can now use this voice with: speechscribe speak 'text' --output file.wav --voice {output.name}"
        )
        click.echo(f"🧪 Test voice quality with: speechscribe test-voice {output.name}")
        click.echo(
            f"🚀 For better naturalness, try: speechscribe train-voice-enhanced '{input_folder}' '{output.name}_enhanced' --quality {quality}"
        )
...
@click.option(
    ...
    default="high",
    type=click.Choice(["low", "medium", "high", "ultra"]),
    help="Quality level for maximum naturalness",
)
@click.option(
    "--min-duration",
    default=2.0,
    help="Minimum audio duration in seconds (default: 2.0 for optimal speech)",
)
@click.option(
    "--max-duration",
    default=60.0,
    help="Maximum audio duration in seconds (default: 60.0 for natural speech)",
)
@click.option(
    "--recursive", "-r", is_flag=True, help="Process subdirectories recursively"
)
...
def train_voice_enhanced(
    ...
    try:
        ...
        click.echo(
            f"🎵 Found {len(audio_files)} audio files for enhanced naturalness training"
        )
        click.echo(
            f"📏 Duration filter: {min_duration:.1f}s - {max_duration:.1f}s (optimized for natural speech)"
        )
        ...
        )  # Sort by closeness to optimal 5-second duration

        # Train the enhanced voice model
        click.echo(
            f"🎭 Training enhanced naturalness voice model from {len(valid_files)} audio files..."
        )
        click.echo(f"📁 Output directory: {output}")
        click.echo(
            f"🌿 Naturalness features: Enhanced pitch variation, breathing patterns, emotional expression"
        )
        ...
        click.echo(
            f"🎤 Test the improved voice with: speechscribe test-voice {output.name}"
        )
        click.echo(
            f"💡 This model focuses on: Natural pitch variation, timing, and emotional expression"
        )
...
def test_voice(ctx, voice_name, text, output):
    ...
    try:
        ...
        click.echo(
            f"🎯 Overall Score: {result['quality_metrics']['overall_score']:.2f}/1.0"
        )
        click.echo(
            f"🌿 Naturalness: {result['quality_metrics']['naturalness_score']:.2f}/1.0"
        )
        click.echo(
            f"🎵 Pitch Quality: {result['quality_metrics']['pitch_score']:.2f}/1.0"
        )
        click.echo(
            f"⏱️  Timing Quality: {result['quality_metrics']['timing_score']:.2f}/1.0"
        )
...
def enhanced_speak(ctx, text, output, quality, emotion, speed, pitch, emphasis):
    ...
    try:
        ...
        output_path = synthesizer.text_to_speech(
            text, output, "default", "coqui_tts", **kwargs
        )
...
def trained_voices(ctx):
    ...
    try:
        ...
        if voices_list:
            ...
            click.echo(
                "💡 Use these voices with: speechscribe speak 'text' --voice <voice_name> --output file.wav"
            )
        else:
            click.echo("  No trained voice models found")
            click.echo(
                "  Train a voice with: speechscribe train-voice <folder> --output <voice_name>"
            )
...
@click.option(
    ...
    default="Happy",
    help="Emotional tone for maximum naturalness",
)
@click.option(
    "--emphasis",
    type=click.Choice(["weak", "normal", "moderate", "strong"]),
    default="strong",
...
@click.pass_context
def expressive_speak(
    ctx, text, output, voice, engine, emotion, emphasis, speed, pitch, quality
):
...
def analyze_audio(input_path, recursive, pattern, min_score):
    ...
    try:
        ...
        click.echo(
            f"🔍 Analyzing {len(audio_files)} audio files for voice training quality..."
        )
        ...
        for i, (audio_file, quality) in enumerate(quality_results, 1):
            ...
            click.echo(
                f"    🎵 Sample Rate: {quality.get('sample_rate', 'Unknown')} Hz"
            )
        ...
        if avg_score >= 0.8:
            click.echo(
                f"   🎉 Excellent audio quality! Use 'ultra' quality for maximum realism."
            )
        elif avg_score >= 0.6:
            click.echo(
                f"   👍 Good audio quality. Use 'high' quality for best results."
            )
        elif avg_score >= 0.4:
            click.echo(
                f"   ⚠️ Fair audio quality. Consider 'medium' quality or better source files."
            )
        else:
            click.echo(
                f"   ❌ Poor audio quality. Consider using higher quality source files."
            )

        click.echo(f"\n🚀 To train a voice model with these files:")
        click.echo(
            f"   speechscribe train-voice {input_path} --output voice_name --quality high"
        )
...
def find_audio_files(input_path: Path, recursive: bool, pattern: str) -> List[Path]:
    ...
    audio_files = [
        f for f in audio_files if f.is_file() and any(f.match(p) for p in patterns)
    ]
