...
class TranscriptionManager:
    ...
    def transcribe_audio(
        ...
        try:
            ...
            if (
                self.model is None
                or getattr(self.model, "model_size", None) != model_name
            ):
            ...
            if no_convert:
                ...
                try:
                    ...
                    logger.warning(
                        f"Audio conversion failed: {e}. Using original file."
                    )
                    prepared_audio = audio_path
            ...
            with tqdm(
                ...
                for segment in segments_iter:
                    ...
                    pbar.update(1)
                    pbar.set_postfix(
                        {"time": f"{segment.end:.1f}s", "segments": len(segments)}
                    )
    ...
    def process_single_file(
        self, audio_path: Path, output_dir: Optional[Path] = None, **kwargs
    ) -> List[Path]:
        ...
        try:
            ...
            logger.info(
                f"Generated {len(output_files)} output files for {audio_path.name}"
            )
            return output_files
    ...
    def batch_transcribe(
        self, audio_paths: List[Path], output_dir: Optional[Path] = None, **kwargs
    ) -> Dict[Path, List[Path]]:
        ...
        with tqdm(
            ...
            for audio_path in audio_paths:
                try:
                    ...
                    # Process single file
                    output_files = self.process_single_file(
                        audio_path, output_dir, **kwargs
                    )
                    results[audio_path] = output_files

                    pbar.set_postfix(
                        {"success": len(results), "failed": len(failed_files)}
                    )
        ...
        logger.info(
            f"Batch processing completed: {successful}/{len(audio_paths)} files successful"
        )
